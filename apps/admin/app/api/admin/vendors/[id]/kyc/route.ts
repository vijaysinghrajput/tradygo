...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const res = await fetch(`${BASE}/admin/vendors/${params.id}/kyc`, { headers, cache: 'no-store' });
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const res = await fetch(`${BASE}/admin/vendors/${params.id}/kyc`, { method: 'POST', headers, body: JSON.stringify(body) });
