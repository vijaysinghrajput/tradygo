...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const res = await fetch(`${BASE}/admin/vendors/${params.id}`, { cache: 'no-store', headers });
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const res = await fetch(`${BASE}/admin/vendors/${params.id}`, { method: 'PATCH', headers, body: JSON.stringify(body) });
