...
export async function PATCH(request: NextRequest, { params }: { params: { bankId: string } }) {
  const accessToken = request.cookies.get('tg_at')?.value;
  const headers: Record<string, string> = { 'Content-Type': 'application/json' };
  if (accessToken) headers['Authorization'] = `Bearer ${accessToken}`;
  const body = await request.json();
  const res = await fetch(`${BASE}/admin/vendors/bank-accounts/${params.bankId}`, { method: 'PATCH', headers, body: JSON.stringify(body) });
...
export async function DELETE(request: NextRequest, { params }: { params: { bankId: string } }) {
  ...
  if (accessToken) headers['Authorization'] = `Bearer ${accessToken}`;
  const res = await fetch(`${BASE}/admin/vendors/bank-accounts/${params.bankId}`, { method: 'DELETE', headers });
  const data = await res.json().catch(() => ({}));
  return NextResponse.json(data, { status: res.status });
}
