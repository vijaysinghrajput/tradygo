...
export async function PATCH(request: NextRequest, { params }: { params: { ruleId: string } }) {
  ...
  const res = await fetch(`${BASE}/admin/vendors/commissions/${params.ruleId}`, { method: 'PATCH', headers, body: JSON.stringify(body) });
...
export async function DELETE(request: NextRequest, { params }: { params: { ruleId: string } }) {
  ...
  if (accessToken) headers['Authorization'] = `Bearer ${accessToken}`;
  const res = await fetch(`${BASE}/admin/vendors/commissions/${params.ruleId}`, { method: 'DELETE', headers });
  const data = await res.json().catch(() => ({}));
  return NextResponse.json(data, { status: res.status });
}
