...
export async function PATCH(request: NextRequest, { params }: { params: { addressId: string } }) {
  ...
  const res = await fetch(`${BASE}/admin/vendors/addresses/${params.addressId}`, { method: 'PATCH', headers, body: JSON.stringify(body) });
...
export async function DELETE(request: NextRequest, { params }: { params: { addressId: string } }) {
  ...
  if (accessToken) headers['Authorization'] = `Bearer ${accessToken}`;
  const res = await fetch(`${BASE}/admin/vendors/addresses/${params.addressId}`, { method: 'DELETE', headers });
  const data = await res.json().catch(() => ({}));
  return NextResponse.json(data, { status: res.status });
}
